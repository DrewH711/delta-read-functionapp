...
def flows_per_day(req: HttpRequest):
    ...
    return HttpResponse(orjson.dumps(flows_per_day), status_code=200, mimetype="application/json")



@bp_delta.route(route="get/total-of-type", auth_level=AuthLevel.FUNCTION, methods=["GET"])
def total_of_type(req: HttpRequest):
    ...
    if not all([study, days, tipe]):
        return HttpResponse("Missing at least one required param: study, days, type", status_code=400)
    ...
    if tipe == "CompletedFlow":
        ...
        return HttpResponse(body=str(subtable.filter(condition).num_rows), status_code=200, mimetype='text/plain')
